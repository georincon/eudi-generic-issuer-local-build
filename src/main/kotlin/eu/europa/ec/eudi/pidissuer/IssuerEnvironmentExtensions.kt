...
import eu.europa.ec.eudi.pidissuer.adapter.out.attestation.academic.*
...
internal object IssuerFactory {
    ...
    context(ctx: Ctx)
    fun academicCredentialInSdJwtVc(
        issuerSigningKey: IssuerSigningKey,
        getAcademicData: GetAttestationAttributes<AcademicCredential>,
    ): IssueAcademicCredential {
        val proofsSupportedSigningAlgorithms =
            ctx.env.readNonEmptySet(
                "issuer.academic.sd_jwt_vc.proofs.supportedSigningAlgorithms",
                JWSAlgorithm::parse,
            )
        val academicCredentialReusePolicy =
            credentialReusePolicy(ctx.env, "issuer.academic.sd_jwt_vc")
        val validity = Duration.parse(ctx.env.getProperty("issuer.academic.sd_jwt_vc.duration", "P365D"))
        val digestHashAlgorithm =
            ctx.env.getProperty<HashAlgorithm>(
                "issuer.academic.sd_jwt_vc.digests.hashAlgorithm",
            ) ?: HashAlgorithm.SHA_256

        val notificationsEnabled =
            ctx.env.getProperty<Boolean>("issuer.academic.sd_jwt_vc.notifications.enabled") ?: true

        return IssueAcademicCredential(
            sdJwtVcSerialization = SdJwtVcSerialization.Compact,
            clock = ctx.clock,
            getAttestationAttributes = getAcademicData,
            issuerSigningKey = issuerSigningKey,
            digestsHashAlgorithm = digestHashAlgorithm,
            deviceBinding =
                DeviceBinding.Required.ts3(
            ...
            credentialReusePolicy = academicCredentialReusePolicy,
            validity = validity,
            validateProof = ctx.validateProof,
            generateNotificationId = ctx.generateNotificationId.takeIf { notificationsEnabled },
            storeIssuedCredential = ctx.storeIssuedCredential,
...
internal fun getAcademicDataFromDatabase(env: Environment): GetAcademicDataFromDatabase {
    val config =
        AcademicDatabaseConfig(
            institutionName = env.getProperty("issuer.academic.institution.name", "Universidad Industrial de Santander"),
            institutionCountry = env.getProperty("issuer.academic.institution.country", "CO"),
            institutionUri = env.getProperty("issuer.academic.institution.uri", "https://www.uis.edu.co"),
            institutionNit = env.getProperty("issuer.academic.institution.nit"),
        )

    val dbUrl = env.getProperty("issuer.uis.db.url") ?: "r2dbc:pool:postgresql://localhost:5434/uis_academic"
    val dbUsername = env.getProperty("issuer.uis.db.username") ?: "uis_academic"
    val dbPassword = env.getProperty("issuer.uis.db.password") ?: "uis_academic"
    ...
    val connectionFactory =
        io.r2dbc.spi.ConnectionFactories.get(
            io.r2dbc.spi.ConnectionFactoryOptions
                .parse(dbUrl)
                .mutate()
                .option(io.r2dbc.spi.ConnectionFactoryOptions.USER, dbUsername)
                .option(io.r2dbc.spi.ConnectionFactoryOptions.PASSWORD, dbPassword)
                .build(),
        )
    ...
    return GetAcademicDataFromDatabase(r2dbcClient, config)
}
